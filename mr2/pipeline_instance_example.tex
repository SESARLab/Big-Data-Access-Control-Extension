...
\begin{enumerate*}[label=\textit{\roman*})]
  \item for vertex \vi{5}, the filtering algorithm produces the set $S_{1}=\{s_{51},s_{52}\}$. Assuming that the dataset owner is ``CT'', the service profile of \s{51} matches \p{1} and $\s{52}$'s one matches \p{2}. For $\s{53}$, there is no policy match and, thus, it is discarded;
