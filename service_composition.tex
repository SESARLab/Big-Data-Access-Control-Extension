...
A proper data management approach must track functional data manipulations across the entire pipeline execution, defining the functional requirements of each service operating on data. To this aim, each node \vi{i}$\in$\V$_S$ is annotated with a label \myGamma(\vi{i}), corresponding to the functional description $F_i$ of the service $s_i$ represented by \vi{i}. $F_i$ describes the functional requirements on the corresponding service $s_i$, such as API, inputs, expected outputs.
...
The second node (\vi{2}) focuses on data enrichment, where additional information from the states of New York and New Hampshire is integrated into the dataset. It requires a data enrichment policy ($\myLambda(v_2)$) to ensure that the added data is relevant and compliant with privacy regulations. The transformation function \TF{2} in $\myGamma(v_2$) is an additive function \TF{a}, which merges and integrates the external data with the existing dataset.
  ...
  Given the \pipelineTemplate we define our \pipeline instantiation technique as a function that takes as input a \pipelineTemplate \iChartFunction and a different sets of candidate services,
  each satisfying the functional requirements of one service operation invocation, and returns as output a \pipelineInstance \iChartFunction.
  In \iChartFunction, every invocations $\vi{i}\in\V$ contains a service instance, and every branching $v\otimes$ is maintained as it is. We formally define our \pipelineInstance as follows.
  ...
  \begin{definition}[Pipeline Instance]\label{def:instance}
    ...
    and for each $\vi{i}\in\V_S$ annotated with policy \P{i} it exists a corresponding \vii{i}$\in$ \Vp$_S$ instantiated with a real service $s_i$, such that the following conditions hold:
  ...
  \begin{itemize}
    ...
    \item \textit{Filtering Algorithm} -
          It matches policy requirements in \tChartFunction against services policy
          and returns as output a set of services that satisfy the requirements.
          Formally, let us consider a set S of candidate services \si, each one having a policy \P{i}.

          Assuming that policy matching are successful for each \s{i}$\in$S. The matching process i successful if \P{i} satisfies \myLambda(\vi{i});
          otherwise, \si is discarded and not considered fore selection.
...
\begin{example}\label{ex:instance}
  ...
  The first node (\vi{1}) is responsible for data anonymization. In the system there are two services that satisfy the functional requirements of the first node, namely $s_1$ and $s_2$.
  ...
  The same logic is applied to the third node (\vi{3}) and any other node in the pipeline template.
  %\end{enumerate*}
