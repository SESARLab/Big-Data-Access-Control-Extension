...
\begin{definition}[Template] \label{def:template}
  ...
  \begin{enumerate}[label=\roman*)]
    ...
    \item \myLambda taht assigns a label \myGamma(\vi{i}), corresponding to a data transformation function to be applied by the service represented by \vi{i}, for each vertex $\vi{i}\in\V$.
  \end{enumerate}
...
\hl{AGGIUNGERE FRASETTA DI COLLEGAMENTO A QUANTO SOTTO.}
...
\hl{SPIEGARE MEGLIO L'OBIETTIVO FUNZIONALE} We also note that each service $s_i$ in \vi{i}$\in$\V\ receives a set of data as input, processes them according to transformation function \F\ annotated on \hl{MANCA LA PAROLA}, and possibly produces a result as output.
