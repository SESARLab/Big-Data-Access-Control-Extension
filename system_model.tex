...
\AJ{
  In our context, the data owner, acting as a user, aims to implement a data pipeline to facilitate analysis or transformation procedures on their data. Policies governing the data are established, which can originate from the user, third-party entities, or regulatory frameworks.
  For instance, a policy may restrict the execution of processing operations to a specific geographic region.
  During the pipeline construction, the user is given the opportunity to select, for each step of the pipeline, from a collection of functionally equivalent candidate services that align with the specified policies.
