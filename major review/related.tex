...
Table \ref{tab:comparative} provides a comparative analysis of existing approaches, highlighting how few industrial solutions compare to our framework according to the following critical features:
\begin{itemize}
...
\begin{table}[t!]
    ...
    \begin{tabular}{lp{3cm}p{3.5cm}p{3cm}p{3cm}p{2.5cm}}
        ...
        \midrule
        \textbf{Microsoft Presidio \cite{microsoft_presidio}} & \cmark, can integrate within cloud-edge pipelines & \tmark, focuses on data redaction & \cmark, compatible with diverse frameworks & \tmark, pre-built PII detectors with configurable policies \\

        \textbf{Apache Ranger \cite{apache_ranger}} & \tmark, limited to mostly cloud settings & \xmark, provides access control rather than service optimization & \cmark, integrates with various platforms & \cmark; highly expressive with fine-grained policy control \\

        \textbf{Paper’s Approach} & \cmark, suitable for distributed, multi-tenant environments & \cmark, enables selection of services that optimize data quality while ensuring data protection & \cmark, applies general data protection annotations & \cmark; flexible metadata annotations enable fine-grained, multi-stage policy enforcement \\
        \bottomrule
