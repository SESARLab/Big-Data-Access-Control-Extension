...
\begin{table}[ht!]
    ...
    \begin{tabular}{p{4cm}p{3cm}p{3.5cm}p{3cm}p{3cm}p{2.5cm}}
        \toprule
        \textbf{Tool/Approach} & \textbf{Supports Cloud-Edge Continuum in Service-Based Pipelines} & \textbf{Optimizes Service Selection for Data Quality and Protection} & \textbf{Framework-Agnostic for Data Protection} & \textbf{Automatic Tuning of Data Protection by Service Profile} & \textbf{Expressiveness} \\
        \midrule
        \textbf{Microsoft Presidio} & Yes, can integrate within cloud-edge pipelines & Limited; focuses on data redaction & Yes, compatible with diverse frameworks & Limited; requires configuration but supports partial automation & Moderate; pre-built PII detectors with configurable policies \\

        \textbf{Apache Ranger} & Limited to mostly cloud settings & No; provides access control rather than service optimization & Yes, integrates with various platforms & No; follows static policy-based enforcement & High; highly expressive with fine-grained policy control \\

        \textbf{Differential Privacy} & Yes, adaptable across pipelines with cloud-edge implementation & No; high privacy can reduce data utility & Yes, adaptable to various frameworks & Limited; requires manual setting of privacy budget & Moderate; flexible for numeric data with controlled noise addition \\

        \textbf{\(k\)-Anonymity} & Yes, adaptable to cloud-edge for basic privacy & No; anonymization compromises data quality & Yes, generalizable & No; manual tuning required for each dataset & Low; lacks flexibility beyond basic anonymization level \\

        \textbf{\(l\)-Diversity} & Yes, adaptable to cloud-edge for basic privacy & No; anonymization reduces data granularity & Yes, generalizable & No; fixed anonymization approach & Low; provides only limited expressiveness with enhanced privacy over \( k \)-anonymity \\

        \textbf{Heuristic Framework (Paper’s Approach)} & Yes, suitable for distributed, multi-tenant environments & Yes, enables selection of services that optimize data quality while ensuring data protection & Yes, applies general data protection annotations & Yes, through heuristic-based tuning that aligns with service profiles and data protection needs & High; flexible metadata annotations enable fine-grained, multi-stage policy enforcement \\
        \bottomrule
