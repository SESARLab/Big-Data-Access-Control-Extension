...
\begin{table}[ht!]
    ...
    \begin{tabular}{lp{3cm}p{3.5cm}p{3cm}p{3cm}p{2.5cm}}
        ...
        \midrule
        \textbf{Microsoft Presidio \cite{microsoft_presidio}} & \cmark, can integrate within cloud-edge pipelines & Limited; focuses on data redaction & \cmark, compatible with diverse frameworks & Limited; requires configuration but supports partial automation & \tmark; pre-built PII detectors with configurable policies \\

        \textbf{Apache Ranger \cite{apache_ranger}} & Limited to mostly cloud settings & \xmark, provides access control rather than service optimization & \cmark, integrates with various platforms & \xmark, follows static policy-based enforcement & \cmark; highly expressive with fine-grained policy control \\

        \textbf{Differential Privacy \cite{dwork2008differential}} & \cmark, adaptable across pipelines with cloud-edge implementation & \xmark, high privacy can reduce data utility & \cmark, adaptable to various frameworks & \tmark; requires manual setting of privacy budget & \tmark; flexible for numeric data with controlled noise addition \\

        \textbf{\(k\)-Anonymity \cite{k-anon}} & \cmark, adaptable to cloud-edge for basic privacy & \xmark, anonymization compromises data quality & \cmark, generalizable & \xmark, manual tuning required for each dataset & \cmark; lacks flexibility beyond basic anonymization level \\

        \textbf{\(l\)-Diversity \cite{l-diversity}} & \cmark, adaptable to cloud-edge for basic privacy & \xmark, anonymization reduces data granularity & \cmark, generalizable & \xmark, fixed anonymization approach & \xmark; provides only limited expressiveness with enhanced privacy over \( k \)-anonymity \\
