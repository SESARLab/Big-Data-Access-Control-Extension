  ...
  \resizebox{\textwidth}{!}{
    \begin{tabular}[t]{cc}
      \begin{tabular}[t]{c|c|l}
        ...
        \vi{1},\vi{2},\vi{3} & \myLambda{(\vi{1})},\myLambda{(\vi{2})},\myLambda{(\vi{3})}        & $p_0$ = \policy{ANY}{dataset}{READ}{ANY}{\tp{0}}                                                \\
...
\\
  \newcommand{\function}[1]{$\scriptstyle\ensuremath{\myLambda({#1}),\myGamma(#1)}$}
  \begin{tikzpicture}[
    ...
    \node[above] at (s1.north)  {\function{1}};
    \node[above] at (s2.north)  {\function{2}};
    \node[below] at (s3.south)  {\function{3}};
